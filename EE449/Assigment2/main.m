...
train_target = matrixfy(10, train_target);
test_target = matrixfy(10, test_target);
...
for i = 1:size(confMat, 1)
    precision(i) = confMat(i, i) / sum(confMat(:, i));  % TP / (TP + FP)
end
...
for i = 1:size(confMat, 1)
    recall(i) = confMat(i, i) / sum(confMat(i, :));  % TP / (TP + FN)
end
...
for i = 1:num_classes
    ...
    per_class_accuracy(i) = (TP + TN) / total_samples;
end
